...
namespace GercStudio.USK.Scripts
{
	public static class PUNHelper
	{
		...
		public static List<string> MatchTargets = new List<string> {"Kills", "Score", "Point Retention"};
...
//					Helper.EnableAllParents(MatchTimer.gameObject);
		...
		public class PauseMenu
		{
			...
			public void ActivateTeamsMenu(bool activeButtons)
			{
				if(activeButtons)
					ActivateButtons();
			...
			public void ActivateNotTeamsMenu(bool activeButtons)
			{
				if(activeButtons)
					ActivateButtons();
...
//			Debug.LogError("best distance | area " + bestDistances.IndexOf(bestDistances.Max()) + " dist " + bestDistances.Max());
		...
		public static List<Controller> GetOnlyTeammates()
		{
			...
			var removedPlayers = allPlayers.Where(player => (PUNHelper.Teams) PhotonNetwork.LocalPlayer.CustomProperties["t"] == player.MyTeam).ToList();
...
//				Debug.LogError(livePlayers.Count);
		...
		public static void CheckPoint(string point, ref Teams lastCapturedTeam, UIManager currentUIManager, RoomManager manager, bool immediately, GameObject realPoint, RawImage screentTarget)
		{
			...
			var redPlayers = capturingPlayers.FindAll(player => (Teams) player.CustomProperties["t"] == Teams.Red);
			var bluePlayers =  capturingPlayers.FindAll(player => (Teams) player.CustomProperties["t"] == Teams.Blue);

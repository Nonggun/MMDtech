...
//			public Animator anim;
		...
		public class Attack
		{
			...
			[Range(1, 5)] public float AttackNoiseRadiusMultiplier = 2;
...
//			{
			...
			IKHelper.PlaceAllIKObjects(weaponController, weaponController.CurrentWeaponInfo[weaponController.SettingsSlotIndex], placeAll, dirObj);
//			}
...
//			}
		...
		public static bool UpdateAttackDirection(WeaponController script, ref RaycastHit raycastHit)
        {
            if (!script.DetectObject && !script.Controller.CameraController.cameraOcclusion && (script.Controller.TypeOfCamera == CharacterHelper.CameraType.ThirdPerson && script.isAimEnabled || script.Controller.TypeOfCamera != CharacterHelper.CameraType.ThirdPerson))
            {
                ...
                {
                    if (script.Controller.TypeOfCamera != CharacterHelper.CameraType.TopDown)
                    {
                    ...
                    else
                    {
                        ...
                        {
	                        if (script.Controller.CameraParameters.FollowCursor)
	                        {
                ...
                for (int i = 0; i < 50; i++)
                {
                    ...
                    if (controller.TypeOfCamera == CharacterHelper.CameraType.TopDown)
                    {
                        ...
                        else
                        {
	                        ...
	                        if (weaponController.Controller.CameraParameters.FollowCursor)
                            {
...
//	                            trajectoryPosition = ProjectileHelper.ComputePositionAtTimeAhead(startPoint.position, velocity, Physics.gravity.y, currentTime);
		...
		public static void SmoothWeponMovement(WeaponController weaponController)
		{
            ...
            else if (!weaponController.isMultiplayerWeapon && weaponController.Controller.smoothWeapons && !weaponController.Controller.AdjustmentScene && weaponController.firstTake && weaponController.Controller.anim.GetBool("HasWeaponTaken") && !weaponController.Controller.anim.GetCurrentAnimatorStateInfo(1).IsName("Take Weapon") &&
...
//                     
//                     (weaponController.Attacks[weaponController.currentAttack].AttackType == WeaponsHelper.TypeOfAttack.Melee && !weaponController.Controller.anim.GetCurrentAnimatorStateInfo(1).IsName("Attack") || 

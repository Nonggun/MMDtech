	...
	public class CreateWeaponWindow : EditorWindow
	{
		...
		private void OnGUI()
		{
			...
			if (hasCreated)
			{
				...
				EditorGUILayout.HelpBox("First of all, add other parameters (like animations, sounds, etc) to the [WeaponController] script" + "\n\n" +
				                        "Then go to the [Tools -> USK -> Adjust] to adjust the IK." + "\n\n" +
				                        "After all, add the weapon to to the [Inventory] script (that is on your character).", MessageType.Info);

   ...
   public class LobbyEditor : Editor
   {
       ...
       public void OnEnable()
       {
           ...
           charactersList = new ReorderableList(serializedObject, serializedObject.FindProperty("Characters"), true, true, true, true)
           {
               ...
               {
                   EditorGUI.LabelField(new Rect(rect.x, rect.y, rect.width, EditorGUIUtility.singleLineHeight),"Character");
               },
...
//                   script.Levels[index].LevelName = EditorGUI.TextField(new Rect(rect.x, rect.y, rect.width / 2, EditorGUIUtility.singleLineHeight), script.Levels[index].LevelName);
           ...
           gameModesList = new ReorderableList(serializedObject, serializedObject.FindProperty("GameModes"), true, true, true, true)
           {
               ...
               onSelectCallback = items => { script.currentMode = items.index; }, 
           ...
           avatarsList = new ReorderableList(serializedObject, serializedObject.FindProperty("DefaultAvatars"), true, true, true, true)
           {
               drawHeaderCallback = rect =>
               {
                   EditorGUI.LabelField(new Rect(rect.x, rect.y, rect.width, EditorGUIUtility.singleLineHeight),"Avatar");
               },
           ...
           switch (script.currentInspectorTab)
           {
               ...
               case 0:
                   ...
                   if (gameMode.CanRespawn)
                   {
                       ...
                       {
                           gameMode.matchTarget = EditorGUILayout.Popup("Match Target", gameMode.matchTarget, new []{PUNHelper.MatchTargets[0], PUNHelper.MatchTargets[1]});
                           
                           if (gameMode.matchTarget > 1)
                               gameMode.matchTarget = 1;
                       }
                       ...
                       switch (gameMode.matchTarget)
                       {
                           ...
                           case 2:
                               ...
                               EditorGUILayout.PropertyField(curModeSerialized.FindPropertyRelative("capturePointTimeout"), new GUIContent("Earn points every (sec)"));
                               break;
                       }
                   }
                   else
...
//                                               "Also, can be used with teams - the team that kills all opponents wins.", MessageType.Info);
                   ...
                   {
                       EditorGUILayout.Space();
                       specificWeaponsList[script.currentMode].DoLayoutList();
                   }
...
//           DrawDefaultInspector();
       ...
       {
           Array.Resize(ref specificWeaponsList, script.GameModes.Count);
           
           for (var i = 0; i < script.GameModes.Count; i++)
           {
               var i1 = i;
               specificWeaponsList[i] = new ReorderableList(serializedObject, serializedObject.FindProperty("GameModes").GetArrayElementAtIndex(script.currentMode).FindPropertyRelative("WeaponsForThisMode"), true, true, true, true)
               {
                   drawHeaderCallback = rect =>
                   {
                       EditorGUI.LabelField(new Rect(rect.x, rect.y, rect.width / 1.5f, EditorGUIUtility.singleLineHeight), "Weapon Prefab");
                   ...
                   onAddCallback = items =>
                   {
                       if (!Application.isPlaying)
                       {
                           script.GameModes[i1].WeaponsForThisMode.Add(new PUNHelper.WeaponSlot());
                       }
                   },
                   ...
                   onRemoveCallback = items =>
                   {
                       if (!Application.isPlaying)
                       {
                           script.GameModes[i1].WeaponsForThisMode.Remove(script.GameModes[script.currentMode].WeaponsForThisMode[items.index]);
                       }
                   },

                   drawElementCallback = (rect, index, isActive, isFocused) =>
                   {
                       script.GameModes[i1].WeaponsForThisMode[index].weapon = (WeaponController) EditorGUI.ObjectField(new Rect(rect.x, rect.y, rect.width / 1.5f, EditorGUIUtility.singleLineHeight),
                           script.GameModes[i1].WeaponsForThisMode[index].weapon, typeof(WeaponController), false);

                       script.GameModes[i1].WeaponsForThisMode[index].slot = EditorGUI.Popup(new Rect(rect.x + rect.width / 1.5f + 10, rect.y, rect.width / 3 - 10, EditorGUIUtility.singleLineHeight), script.GameModes[i1].WeaponsForThisMode[index].slot, new[] {"1", "2", "3", "4", "5", "6", "7", "8"});
                   }
               };
           }
       }

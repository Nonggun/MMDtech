...
//			}
			...
			switch (script.topInspectorTab)
			{
				case 0:
					...
					script.movementBottomInspectorTab = GUILayout.Toolbar(script.movementBottomInspectorTab, new[] { "See Area", "Behaviour"});
					...
					switch (script.currentMovementInspectorTab)
					{
						case 0:
							...
							EditorGUILayout.HelpBox("◆ If this checkbox is active, the enemy's movement and speed are based on Root Motion animations." + "\n\n" +
							                        "◆ If not, you can adjust enemy's speed manually.", MessageType.Info);
							...
							if (script.projectSettings)
							{
								...
								EditorGUILayout.HelpBox("Use the tag to set footsteps sounds for this enemy. " + "\n\n" +
								                        "Step sounds are set in the surface presets (Presets -> Surfaces).", MessageType.Info);
						...
						case 1:
							...
							EditorGUILayout.PropertyField(serializedObject.FindProperty("HorizontalAngleToSee"), new GUIContent("Horizontal Angle"));
						...
						case 3:
							...
							if (!script.UseStates)
								...
								EditorGUILayout.HelpBox(
								                        "◆ If the enemy sees/hears a player for a while or the player has shot him a few times, " +
								                        ...
								                        " And after that returns to a waypoint.", MessageType.Info);
						...
						case 1:
							...
							switch (script.Attacks[0].AttackType)
							{
								case AIHelper.AttackTypes.Bullets:
									EditorGUILayout.Space();
									EditorGUILayout.BeginVertical("HelpBox");
									EditorGUILayout.PropertyField(serializedObject.FindProperty("Weapon"), new GUIContent("Weapon"));
									EditorGUILayout.EndVertical();
									EditorGUILayout.Space();
									EditorGUILayout.BeginVertical("HelpBox");
									...
									EditorGUILayout.Space();
									EditorGUILayout.BeginVertical("HelpBox");
									EditorGUILayout.PropertyField(serializedObject.FindProperty("Attacks").GetArrayElementAtIndex(0).FindPropertyRelative("HandsIdleAnimation"), new GUIContent("Idle animation for weapon"));
									EditorGUILayout.PropertyField(serializedObject.FindProperty("Attacks").GetArrayElementAtIndex(0).FindPropertyRelative("HandsAttackAnimation"), new GUIContent("Attack animation for weapon"));
									EditorGUILayout.EndVertical();
									EditorGUILayout.Space();
									EditorGUILayout.BeginVertical("HelpBox");
									EditorGUILayout.HelpBox("If the enemy finds suitable cover (close enough to yourself and to the player), he will hide behind it." + "\n\n" +
									                        "To create a cover you should add to object the [Surface] script and active [IsCover] checkbox.", MessageType.Info);
				...
				case 2:
					...
					if (script.isHuman && (!script.BodyParts[0] || script.BodyParts[0] && !script.BodyParts[0].GetComponent<Rigidbody>()))
					{
						EditorGUILayout.Space();
#if !UNITY_2018_3_OR_NEWER
...
//
//                var animator = script.gameObject.GetComponent<Animator>();
//                
//                script.BodyParts[0] = SetFieldValue(ragdollWindow, "pelvis", animator.GetBoneTransform(HumanBodyBones.Hips));
//                script.BodyParts[1] = SetFieldValue(ragdollWindow, "middleSpine", animator.GetBoneTransform(HumanBodyBones.Spine));
//                script.BodyParts[2] = SetFieldValue(ragdollWindow, "head", animator.GetBoneTransform(HumanBodyBones.Head));
//                script.BodyParts[3] = SetFieldValue(ragdollWindow, "leftHips", animator.GetBoneTransform(HumanBodyBones.LeftUpperLeg));
//                script.BodyParts[4] = SetFieldValue(ragdollWindow, "leftKnee", animator.GetBoneTransform(HumanBodyBones.LeftLowerLeg));
//                SetFieldValue(ragdollWindow, "leftFoot", animator.GetBoneTransform(HumanBodyBones.LeftFoot));
//                script.BodyParts[5] = SetFieldValue(ragdollWindow, "rightHips", animator.GetBoneTransform(HumanBodyBones.RightUpperLeg));
//                script.BodyParts[6] = SetFieldValue(ragdollWindow, "rightKnee", animator.GetBoneTransform(HumanBodyBones.RightLowerLeg));
//                SetFieldValue(ragdollWindow, "rightFoot", animator.GetBoneTransform(HumanBodyBones.RightFoot));
//                script.BodyParts[7] = SetFieldValue(ragdollWindow, "leftArm", animator.GetBoneTransform(HumanBodyBones.LeftUpperArm));
//                script.BodyParts[8] = SetFieldValue(ragdollWindow, "leftElbow", animator.GetBoneTransform(HumanBodyBones.LeftLowerArm));
//                script.BodyParts[9] = SetFieldValue(ragdollWindow, "rightArm", animator.GetBoneTransform(HumanBodyBones.RightUpperArm));
//                script.BodyParts[10] = SetFieldValue(ragdollWindow, "rightElbow", animator.GetBoneTransform(HumanBodyBones.RightLowerArm));
//
...
//        private Transform SetFieldValue(ScriptableWizard obj, string name, Transform value)
//        {
//            if (value == null)
//            {
//                return null;

...
namespace GercStudio.USK.Scripts
{
	...
	public class Controller : MonoBehaviour
	{
		...
		private bool isCharacterInLobby;
...
//						transform.rotation = anim.rootRotation;
		...
		private void Awake()
		{
			...
			else if (CameraParameters.ActiveTP && TypeOfCamera == CharacterHelper.CameraType.ThirdPerson) TypeOfCamera = CharacterHelper.CameraType.ThirdPerson;
			else if (CameraParameters.ActiveTD && TypeOfCamera == CharacterHelper.CameraType.TopDown) TypeOfCamera = CharacterHelper.CameraType.TopDown;
		...
		void OnEnable()
		{
			...
			if (firstTake)
			{
				...
				if ((Application.isMobilePlatform || projectSettings.mobileDebug) && !UIManager.UIButtonsMainObject)
				{
					...
					UIManager.cameraStickOutline = Instantiate(UIManager.uiButtons[16].gameObject, UIManager.UIButtonsMainObject.transform, true);
					UIManager.cameraStick = Instantiate(UIManager.uiButtons[15].gameObject, UIManager.UIButtonsMainObject.transform, true);
					...
					if (gameManager)
					{
						...
						{
							UIManager.uiButtons[9].onClick.AddListener(gameManager.Pause);
							UIManager.uiButtons[9].gameObject.SetActive(projectSettings.ButtonsActivityStatuses[10]);
						}
						...
						if (UIManager.uiButtons[12])
						{
							...
							UIManager.uiButtons[12].gameObject.SetActive(projectSettings.ButtonsActivityStatuses[18] && !CharacterSync);
						}
					}
					
					for (var i = 0; i < 15; i++)
					{
						UIManager.buttonsWereActive[i] = UIManager.uiButtons[i].gameObject.activeSelf;
					}
				}
...
//			Debug.DrawRay(BodyObjects.Hips.position + transform.forward * 2, Vector3.down * 10, anim.GetBool("OnFloorForward") ? Color.green : Color.red);
		...
		void GetLocomotionInput()
		{
			...
			if (!isPause)
			{
				...
				{
					CharacterHelper.CheckMobileJoystick(UIManager.moveStick, UIManager.moveStickOutline, ref touchId, projectSettings, ref MobileTouchjPointA, ref MobileTouchjPointB, ref MobileMoveStickDirection, this);
...
//						
		...
		void CheckHealth()
		{
			...
			if (PlayerHealth <=0)
			{
				...
				UIManager.CharacterUI.MainObject.SetActive(false);
				WeaponManager.canvas.gameObject.SetActive(false);
...
//			Debug.DrawRay(BodyObjects.Head.position,new Vector3(CheckCollisionVector.x, checkDir.y, CheckCollisionVector.z) *10, Color.green);
			
			if (Physics.Raycast(BodyObjects.Head.position, new Vector3(CheckCollisionVector.x, checkDir.y, CheckCollisionVector.z), out distanceInfo, CheckCollisionVector.magnitude * 10, Helper.layerMask()))
			{
				if (!distanceInfo.collider || distanceInfo.collider && !distanceInfo.collider.isTrigger)
				{
					if (TypeOfCamera == CharacterHelper.CameraType.FirstPerson)
					{
						...
						else
						{
							...
							if (canMoveTimer > 0.1f) isObstacle = false;
						}
					}
					else
					{
						if (distanceInfo.distance < 2)
						{
							canMoveTimer = 0;
							isObstacle = true;
						}
...
//						SmoothIKSwitch = 0;
		...
		public void ChangeCameraType()
		{
			...
			{
				if (WeaponManager.WeaponController && !WeaponManager.WeaponController.isAimEnabled ||
				    WeaponManager.WeaponController && WeaponManager.WeaponController.isAimEnabled && WeaponManager.WeaponController.activeAimTP)
			...
			{
				if (WeaponManager.WeaponController && !WeaponManager.WeaponController.isAimEnabled ||
				    WeaponManager.WeaponController && WeaponManager.WeaponController.isAimEnabled && WeaponManager.WeaponController.activeAimFP)
					CharacterHelper.SwitchCamera(TypeOfCamera, CharacterHelper.CameraType.FirstPerson, this);
				else if(WeaponManager.WeaponController && !WeaponManager.WeaponController.activeAimFP && WeaponManager.WeaponController.isAimEnabled)
				{
					WeaponManager.WeaponController.Aim(true, false, false);
		...
		{
			if (!anim.GetBool("Aim") && (anim.GetCurrentAnimatorStateInfo(0).IsName("Walk_Forward") || anim.GetCurrentAnimatorStateInfo(0).IsName("Run_Forward")) || anim.GetCurrentAnimatorStateInfo(0).IsName("Crouch_Walk_Forward"))
			{

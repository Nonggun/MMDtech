...
namespace GercStudio.USK.Scripts
{
    ...
    public class WeaponController : MonoBehaviour
    {
        ...
        public bool activeAimTP = true;
        public bool activeAimFP = true;
        ...
        private void OnEnable()
        {
            ...
            {
                Controller.UIManager.uiButtons[3].gameObject.SetActive(Attacks.Count > 1 || Attacks[currentAttack].AttackType == WeaponsHelper.TypeOfAttack.Bullets && Attacks[currentAttack].BulletsSettings[0].Active && Attacks[currentAttack].BulletsSettings[1].Active);
        ...
        public void Attack(bool isAttack, string type)
        {
            if (isAttack && !isReloadEnabled && !Controller.isPause && !Controller.CameraController.cameraPause && canAttack && !WeaponManager.isPickUp && Attacks[currentAttack].curAmmo > 0)
            {
                ...
                if (Controller.TypeOfCamera == CharacterHelper.CameraType.ThirdPerson && !isAimEnabled && activeAimTP && !DetectObject && aimForAttack)
                {
                    Aim(false, false, false);
            ...
            else
            {
                switch (type)
                {
                    case "Single":
                    {
                        ...
                        {
                            if (rateOfAttackTimer > Attacks[currentAttack].RateOfAttack / (Controller.TypeOfCamera == CharacterHelper.CameraType.ThirdPerson ? 2 : 1) && activateMeleeTimer)
                            {
                                MeleeAttackOff();
                            }
                        }
                ...
                if (Attacks[currentAttack].AttackSpawnPoint)
                {
                    ...
                    if (WeaponsHelper.UpdateAttackDirection(this, ref Hit))
                    {
                        ...
                        if (Hit.transform && Hit.transform.GetComponent<BodyPartCollider>())
                        {
                            ...
                            {
                                var controller = bodyColliderScript.Controller;
                                if (!isMultiplayerWeapon)
                                {
                                    switch (Controller.CanKillOthers)
                                    {
                                        case PUNHelper.CanKillOthers.OnlyOpponents:
                                            if (controller.MyTeam != Controller.MyTeam || controller.MyTeam == Controller.MyTeam && Controller.MyTeam == PUNHelper.Teams.Null)
                                            {
                                                if (controller.PlayerHealth - Attacks[currentAttack].weapon_damage * bodyColliderScript.damageMultiplayer <= 0 && Controller.CharacterSync)
                                                {
                                                ...
                                                controller.Damage((int)(Attacks[currentAttack].weapon_damage * bodyColliderScript.damageMultiplayer), Controller.CharacterName, WeaponImage, Controller.oneShotOneKill);
                                            }
                                            break;
...
//        IEnumerator MeleeAttackTimeOut()
//        {
//            while (true)
//            {
//                if (Controller.anim.GetCurrentAnimatorStateInfo(0).IsName("Melee"))
//                {
//                    rateOfAttackTimer = 0;
//                    activateMeleeTimer = true;
//                    StopCoroutine("MeleeAttackTimeOut");
...
//        }
        ...
        public void Aim(bool instantly, bool notSendToMultiplayer, bool gamepadInput)
        {
            if (!isMultiplayerWeapon)
            {
                if (Controller.TypeOfCamera == CharacterHelper.CameraType.ThirdPerson && !activeAimTP ||
                    Controller.TypeOfCamera == CharacterHelper.CameraType.FirstPerson && !activeAimFP || Controller.CameraController.cameraOcclusion ||
            ...
            if (Controller.TypeOfCamera == CharacterHelper.CameraType.ThirdPerson && Attacks[currentAttack].AttackType != WeaponsHelper.TypeOfAttack.Melee || Controller.TypeOfCamera != CharacterHelper.CameraType.ThirdPerson)
            {
                ...
                {
                    Debug.LogWarning("You haven't set the position of the hands for aiming. Go to the [Tools -> USK -> Adjust] to do that.", gameObject);
                }
            }
            else
            {

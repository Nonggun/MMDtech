    ...
    public class CharacterSync : MonoBehaviourPun, IPunObservable
    {
        ...
        #region StartMethods
            ...
            void Start()
            {
                ...
                controller.MyTeam = (PUNHelper.Teams) photonView.Controller.CustomProperties["t"];
                ...
                if (!photonView.IsMine & PhotonNetwork.IsConnected)
                {
                    ...
                    controller.CameraController.Crosshair.gameObject.SetActive(false);
                    ...
                    {
                        if ((PUNHelper.Teams) PhotonNetwork.LocalPlayer.CustomProperties["t"] != controller.MyTeam ||
                            (PUNHelper.Teams) PhotonNetwork.LocalPlayer.CustomProperties["t"] == controller.MyTeam && controller.MyTeam == PUNHelper.Teams.Null)
                        {
                ...
                else
                {
                    ...
                    {
                        if (RoomManager)
                            controller.UIManager.uiButtons[9].onClick.AddListener(RoomManager.Pause);
                    }
...
//            Debug.LogError("change weapon event");
        ...
        public void Destroy()
        {
            ...
            foreach (var player in PlayersList)
            {
                if (controller.KillerName == player.NickName && controller.KillerName != photonView.Owner.NickName)
                {
                    ...
                    {
                        int currentKills;
                        switch ((PUNHelper.Teams) player.CustomProperties["t"])
                        {
            ...
            //match stats here !!!
            var tempScript = Instantiate(RoomManager.currentUIManager.MultiplayerGameRoom.MatchStatsPrefab.gameObject, RoomManager.currentUIManager.MultiplayerGameRoom.MatchStats.KillStatsContent).GetComponent<UIPlaceholder>();
        ...
        void CalculatePlayerPlace(RoomManager roomManager)
        {
            ...
            {
                var numberOfLivePlayers = PhotonNetwork.PlayerList.Where(player => (PUNHelper.Teams)player.CustomProperties["t"] == controller.MyTeam).Count(player => (int) player.CustomProperties["d"] == 0);
                numberOfLivePlayers++;
                PhotonNetwork.LocalPlayer.SetCustomProperties(new Hashtable {{"pl", numberOfLivePlayers}});
            }
        }
        ...
        public void OnPhotonSerializeView(PhotonStream stream, PhotonMessageInfo info)
         {
             ...
             else
             {
                 ...
                 controller.CharacterController.center = new Vector3(controller.CharacterController.center.x, (float) stream.ReceiveNext(), controller.CharacterController.center.z);
